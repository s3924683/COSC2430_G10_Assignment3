...
router.route("/signin").get(async (req, res) => {
    ...
    try {
        ...
        if (!shipper) {
            return res.render('shipperSignin', { message: 'Invalid username or password' });
        }
        ...
        console.error(error);
        res.render('shipperSignin', { message: 'An error occurred. Please try again later.' });
    }
});
router.route("/signup").get(async (req, res) => {
    ...
    try {
        ...
        if (existingShipper) {
            return res.render('shipperSignup', { message: 'Shipper Username is already taken' });
        }
